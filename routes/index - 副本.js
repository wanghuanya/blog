...
router.get('/', function(req, res) {
	...
	// 查询文章
	connection.query('select * from articles'+where,function(error,results1){
		if(error){
			...
			connection.query('select * from types',function(error,results2){
				if(error){
					...
					connection.query('select count(*) as total from articles',function(error,results3){
				        if (error) {
				            console.log('获取数据失败');
				        } else { 
				            var page = {};
				            ...
				            // 4.查询数据
				            connection.query('select * from articles limit '+(page.now-1)*page.every+','+page.every, function(error, results4) {
				                if (error) {
				                    console.log('查询失败', error);
				                    res.send('服务器内部错误，请联系管理员...');
				                } else {
				                    res.render('index', { results1: results1,results2:results2,moment:require('moment'),page:page,admin:req.session.admin });
				                }
				            })
				        }
				    })
